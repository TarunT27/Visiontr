...
> `localStorage['vtr:first-run-mission:v1'] === 'suppressed'`, written **only**
> by the "Don't show this again" checkbox → the per-session
> `sessionStorage['vtr:first-run-mission-session:v1'] === 'dismissed'`, written
...
> durable in this app (`vtr:layer-state:v2`, written by
...
> mode/density, `vtr:detection-allocation:v1`, 3D models, feather, and above all
...
> `VTR_REALTIME_TOOLS` is **byte-identical to `main`** and pinned by sha256 in
...
>   evicted vessel-record picks and `vtr-trail:*` remain no-ops. CCTV choices
...
>   tracked-lane card through `vtrLabelModel` and `_trackedDisplayCached`; the
...
>   state, airline/type, and plausible-route formatting in `vtrLabelModel`; the
...
>   and mapped installations write explicit `vtrLabelModel` objects and expose
>   `vtrDisplayPosition` getters backed by their layer-owned frame/display cache.
...
- **CCTV v2 foundation:** a pitched
  ...
  `visonTR.cctv.calibration.v2` (wiped clean, no v1 import); a panel-only CAL badge shows
...
`VTR MIC` button (bottom UI) starts an OpenAI Realtime session over WebRTC:
...
- **Scene context** (`get_entity_context`): selected entity from the context store, or visible entities ranked by distance to the view target (≤100km altitude); plus basemap context — view-target picking, 7-point viewport sampling, view-scale classification (global/continental/regional/metro/city/local), reverse geocoding (center ≤750km, viewport samples ≤3,000km), Google Places Nearby via `/api/google/nearby-places` (≤25km), known-landmark matching against `CITY_POIS`, coarse country inference fallback. Context assembly is capped at 1.5s with cache fallbacks; caches are deduped in-flight.
  - **Every selectable layer writes that one slot** (2026-08-21). The tracking layers — flights, military, satellites — publish selection on their own awareness lane (`vtr:awareness-subject-selected`, consumed by the readout card and the Contacts panel) and for the life of the voice tools never wrote the shared slot, so `scope:'selected'` silently answered `in_view` with a contact plainly selected on screen. They now call `selectTrackedSubjectContext` / `refreshTrackedSubjectContext` / `clearTrackedSubjectContext` (`src/data/contextStore.js`) on select / poll / deselect. **Invariants: (a)** that write path must NOT dispatch `vtr:entity-selected` — tracking layers already own an event lane and a second one makes two surfaces fight over one subject; **(b)** exactly one record per tracking layer, because a frozen snapshot of a moved contact must never reach the visible-entity scan; **(c)** precedence is recency, not layer — one slot, last selection wins, so clicking an overlay entity supersedes a tracked plane as the voice subject while the plane stays tracked. Satellites refresh on the 1 s propagation beat, not per frame.
...
- **Model tier + spend guard** (`src/voice/voiceCost.js`, August 2026): the voice heading row carries a `STD`/`MINI` toggle and a running session-cost readout (`~$0.42`).
  - **Tier selection.** `standard` = `gpt-realtime-2` (default), `mini` = `gpt-realtime-2.1-mini` (~3× cheaper per audio token). The client sends `?tier=` to `/api/realtime/token`; the endpoint resolves it through the shared registry, so an unknown, empty, or hostile value falls back to `standard` rather than reaching OpenAI as a model id. Responses echo `X-VTR-Voice-Tier` / `X-VTR-Voice-Model` (plus `X-VTR-Voice-Tier-Fallback: 1` when a bogus tier was downgraded). Persisted at `visonTR.voiceCost.tier`.
...
- **Diagnostics**: every client/server event is posted to `/api/realtime/debug-log` and appended to `.vtr-logs/realtime-conversations.jsonl` (gitignored) with secret/image redaction; last 30 errors persist in `localStorage` (`vtr-realtime-errors`); `window.__vtrVoiceCommands.getDiagnostics()` in the console.
...
- Runtime entry: `src/main.js` calls `initAnnotations({ viewer, tileset })`, exposes `window.__vtrAnnotations`, and passes the engine into the voice action runner.
...
- Console/dev API: `window.__vtrAnnotations.tour()`, `.demo()`, `.annotate()`, `.clear()`, `.count()`, and `.list()` are the deterministic no-mic test surface.
...
- Length changes are eased, never snapped, and the ease is placed so it never fights the flaps (`.vtr-flap-sizing`): a GROWING label reserves its columns as the cells go in and eases at the START; a SHRINKING one holds full width for the whole cascade and eases at SETTLEMENT.
...
- Panel POSITION keys are versioned `v8` (`visonTR.v8.panelPos.<id>`); collapsed-state keys remain `v6`. The one-time position reset clears stale DISPLAY placements that could overlap the Context rail.
...
- VTR MIC control is a glass capsule (var(--glass-bg), blur(24px) saturate(1.4), 999px radius; panel radius in error state).
...
- Voice control UI (`#vtr-voice-control`) shows status states OFF / CONNECTING / LISTENING / EXECUTING / ERROR.
...
- **3D models** have durable storage — `vtr:layer-state:v2` in local storage,
...
- CCTV calibration persists at `visonTR.cctv.calibration.v2` (v2 rebuild;
...
- Draggable panel positions persist at `visonTR.v7.panelPos.<panel-id>` (collapsed states at `visonTR.v6.panelCollapsed.<panel-id>`).
- Legacy draggable-panel position keys may remain in local storage for backward compatibility, but the map-mode right rail ignores them; collapsed states still persist at `visonTR.v6.panelCollapsed.<panel-id>`.
...
- Traffic runs in `sim` mode (white dots, hardcoded speeds) unless `TOMTOM_API_KEY`
  ...
  (`.vtr-cache/tomtom/`, 120 s TTL, `TOMTOM_DAILY_TILE_BUDGET` default 40k/day),
...
- Voice debug log: `tail -f .vtr-logs/realtime-conversations.jsonl` (gitignored).

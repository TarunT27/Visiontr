# Contributing to VisonTR

Thanks for being here. VisonTR is an open foundation for live spatial intelligence in the browser, and it gets better when more people run it, break it, and extend it.
...
```bash
git clone https://github.com/TarunT27/Visiontr.git
cd Visiontr
nvm install 24.14.0
nvm use 24.14.0
npm install
...
- **🎙️ Extend voice control.** Voice tools are declared server-side (`VTR_REALTIME_TOOLS` in `vite.config.js`) and executed client-side (`src/voice/vtrActions.js`). Keep the tool surface tight and the responses honest (confirm only what actually happened).
